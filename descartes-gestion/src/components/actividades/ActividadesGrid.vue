...
import { ref } from 'vue'
import type { ActividadFila } from '@/config/actividades-columns'
...
function indicadorFila(index: number, fila: ActividadFila) {
  if (fila._nuevo) return '*'
  if (index === props.indiceSeleccionado) return '>'
  return ''
}

function onRowDblClick(index: number, fila: ActividadFila) {
  if (fila._nuevo) {
    emit('nuevo')
    return
  }
...
<template>
  <div class="grid-wrap" :class="{ 'filter-menu-open': filterMenuOpen }">
    ...
    <table v-else class="actividades-grid">
      <thead>
        <tr>
          <th class="col-ind"></th>
