...
<template>
  <div class="grid-wrap" :class="{ 'filter-menu-open': filterMenuOpen }">
    ...
    <table v-else class="agrupaciones-grid">
      <thead>
        <tr>
          <th class="col-ind"></th>
...
.agrupaciones-grid th,
.agrupaciones-grid td {
...
.agrupaciones-grid tbody tr {
  cursor: pointer;
}

.agrupaciones-grid tbody tr.selected {
  background: #dbeafe;
}

.agrupaciones-grid tbody tr.nuevo {
  background: #fefce8;
}

.agrupaciones-grid tbody tr:nth-child(even):not(.selected):not(.nuevo) {
  background: #f8fafc;
}
