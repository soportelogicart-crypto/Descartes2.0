...
const items = ref<ProveedorContacto[]>([])
...
const puedeGuardar = computed(() => puedeEscribir.value && !saving.value && !!props.proveedorCodigo)
...
function cargarEnFormulario(item: ProveedorContacto) {
...
async function cargar(seleccionarPrimera = true) {
  ...
  try {
    const { data } = await api.get(
      `/api/mantenimiento/proveedores/${encodeURIComponent(props.proveedorCodigo)}/contactos`
    )
...
function seleccionar(item: ProveedorContacto) {
  esNueva.value = false
...
async function onGuardar() {
  ...
  try {
    ...
    if (esNueva.value || !seleccion.value) {
      ...
      await api.put(
        `/api/mantenimiento/proveedores/${encodeURIComponent(props.proveedorCodigo)}/contactos/${encodeURIComponent(num)}/${nroLin}`,
        body
      )
    }
  ...
  <div v-if="open" class="overlay" role="dialog" aria-modal="true" @click.self="emit('cerrar')">
    <div class="modal">
      <header class="modal-header">
        <h3>Contactos — {{ proveedorCodigo }}</h3>
