...
import {
  MACROFAMILIA_CAMPOS_OBLIGATORIOS,
  camposMacrofamiliaObligatoriosVacios,
  clonarMacrofamilia,
  macrofamiliaVacia,
  payloadMacrofamilia,
  validarMacrofamiliaObligatorios,
...
const filas = computed<MacrofamiliaFila[]>(() => {
  ...
  const filtradas = aplicarFiltrosColumnas(datos, filtros.value) as MacrofamiliaFila[]
  if (!puedeCrear.value) return filtradas
  return [...filtradas, macrofamiliaVacia()]
})
...
async function abrirFicha(index?: number) {
  ...
  if (!fila || fila._nuevo || fila.codigo == null || fila.codigo === '') {
    mensaje.value = 'Seleccione una macrofamilia existente'
    return
  }
...
async function onGuardarFicha() {
  const vacios = camposMacrofamiliaObligatoriosVacios(ficha.value)
  if (vacios.length > 0) {
    const key = vacios[0]
    const label = MACROFAMILIA_CAMPOS_OBLIGATORIOS.find((c) => c.key === key)?.label ?? key
    const msg = validarMacrofamiliaObligatorios(ficha.value) ?? `El campo "${label}" es obligatorio.`
    mensaje.value = msg
  ...
  if (esNuevo.value) {
    ...
    if (existe) {
      const msg = `Ya existe una macrofamilia con el codigo "${codigo}".`
      mensaje.value = msg
  ...
  try {
    if (esNuevo.value) {
      const creado = await crear(payloadMacrofamilia(ficha.value))
      mensaje.value = 'Macrofamilia creada correctamente'
      await cargar()
      const idx = filas.value.findIndex((p) => String(p.codigo) === String(creado.codigo))
      if (idx >= 0) await abrirFicha(idx)
      else volverAlGrid()
    } else {
      await actualizar(codigo, payloadMacrofamilia(ficha.value))
    ...
    const msg = extractApiError(e, 'No se pudo guardar la macrofamilia')
    mensaje.value = msg
...
async function confirmarBorrarFicha() {
  ...
  try {
    ...
    mensaje.value = 'Macrofamilia eliminada'
    await cargar()
    volverAlGrid()
  } catch (e: unknown) {
  ...
  <section class="macrofamilias-view">
    ...
    <template v-else>
      ...
      <ConfirmDialog
        ...
        title="Eliminar macrofamilia"
        message="Va a eliminar esta macrofamilia de la base de datos. Esta accion no se puede deshacer."
