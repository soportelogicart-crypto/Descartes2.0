...
const filaNuevaDraft = ref<AgrupacionFila>(agrupacionVacia())
...
const filas = computed<AgrupacionFila[]>(() => {
  ...
  const filtradas = aplicarFiltrosColumnas(filasTodas.value, filtros.value) as AgrupacionFila[]
...
function actualizarFila(_index: number, fila: AgrupacionFila) {
...
async function onGuardar() {
  ...
  const errorValidacion = validarAgrupacion(fila)
  ...
  try {
    ...
    const payload = payloadAgrupacion(fila)

    if (fila._nuevo) {

      await crear(payload)

      mensaje.value = 'Agrupacion creada'
    ...
    mensaje.value = extractApiError(e, 'No se pudo guardar la agrupacion')
